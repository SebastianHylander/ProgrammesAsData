...
let rec freevars e : string list =
    ...
    | Let (vars, ebody) ->
        let rec freevars_let = fun (lst : (string*expr) list)  ->
                            match lst with
                            | [] -> []
                            | (var, v) :: xs -> union (minus (freevars_let xs, [var]), freevars v)
        let rec var_names = fun lst ->
            match lst with
            | [] -> []
            | (var, _)::xs -> var :: var_names xs 

        union (freevars_let vars , minus (freevars ebody, var_names vars))
    (*| Let(x, erhs, ebody) -> 
